...
static void load_categories_nolock(header_info *hi) {
    ...
    if (strstr(hi->fname(), "Explorer")) {
        //_objc_inform("load_categories_nolock:%s", hi->fname());
    }
    ...
    auto processCatlist = [&](category_t * const *catlist) {
        for (unsigned i = 0; i < count; i++) {
            ...
            if (strstr(hi->fname(), "Explorer")) {
                //_objc_inform("load_categories_nolock:%s, %s", cat->name, cls->nonlazyMangledName());
            }
...
{
    //_objc_inform("load_images:%s", path);
...
# endif
    ...
    for (EACH_HEADER) {
        //_objc_inform("map_images:%s", hi->fname());
        ...
        for (i = 0; i < count; i++) {
            ...
            static int isExplorer = 1;
            if (strstr(hi->fname(), "Explorer")) {
                if (isExplorer == 1) {
                    isExplorer = 0;
                    //_objc_inform("map_images:%s", hi->fname());
                }
                //_objc_inform("map_images:%s", cls->nonlazyMangledName());
            }
